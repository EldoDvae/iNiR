...
flatpak override --user --filesystem=xdg-config/gtk-3.0:rw --filesystem=xdg-config/gtk-4.0:rw
...
if $QS_INSTALL; then
  v sudo dnf install quickshell niri --setopt=install_weak_deps=True --skip-unavailable -x waybar
else
  log_warning "Could not install Quickshell"
  v sudo dnf install niri --setopt=install_weak_deps=True --skip-unavailable -x waybar
fi
