...
- **Settings Easy/Advanced mode tooltip stretching off-screen**: the title-bar mode toggle tooltip was too verbose and used default bottom placement near the top-right controls, which made it spill out of view. The copy is now shorter and the tooltip is anchored to the left of the toggle so it stays readable.
